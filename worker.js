export default {
  async fetch(request, env) {
    ...
    // 2. HANDLE BROWSER "PREFLIGHT" (Fixes the CORS block)
    if (method === "OPTIONS") {
    ...
    try {
      ...
      if (method === "POST" && url.pathname === "/manual-update") {
        ...
        return new Response(JSON.stringify({ success: true, msg: "Vault Updated" }), { headers });
      }
      ...
      if (!tornData.members) throw new Error("Invalid Torn API Response");
      ...
      for (const id of Object.keys(tornData.members)) {
        ...
        membersList.push({ id, name: m.name, level: m.level, status_desc: m.status.description, total: spy ? spy.total : 0 });
      }
